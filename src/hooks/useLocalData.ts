...
export const useLocalData = () => {
  ...
  const { 
    ...
    queryKey: ['cargasByLocal'],
    queryFn: async () => {
      try {
        const apiUrl = 'http://10.10.10.251:8890/api/inventory/satelites/cargabylocal';
  ...
  useEffect(() => {
    if (isLoading || isRefetching) {
      ...
      }, 300); // Aceleramos un poco la animación
      
      return () => {
        clearInterval(interval);
        if (!isLoading && !isRefetching) {
          // Aseguramos que llegue a 100% cuando termina la carga
          setLoadingProgress(100);
        }
      };
    } else {
      // Garantizamos que llegue a 100% cuando no está cargando
      setLoadingProgress(100);
    }
  ...
  useEffect(() => {
    if (error) {
      ...
      if (errorMessage.includes('Failed to fetch')) {
        ...
        toast.error('Error al cargar los datos de locales', {
  ...
  const totalCargas = locales?.reduce((sum, local) => sum + local.recordsByLocal, 0) || 0;
  const promedioCargas = locales?.length ? totalCargas / locales.length : 0;
